...
import { normalizeEntity, renderString } from '~/lib/core';
      ...
      <div className="flex-1 flex items-center gap-2">
        {selectedCount > 0 ? (
          <>
            ...
            <Button variant="outline" size="sm" className="text-red-600 border-red-200 bg-red-50 hover:bg-red-100 gap-2" onClick={() => { if (confirm(t('entity:bulk_delete_confirm', { count: selectedCount }))) onBulkDelete(); }}><Trash2 size={14} /> <span className="hidden sm:inline">{t('common:delete')}</span></Button>
          </>
        ) : <span className="text-sm text-slate-500 ml-2">{t('entity:select_items')}</span>}
      </div>
      <div className="flex items-center gap-2">
        ...
        {onImportAI && normalized.features.import !== false && normalized.features.creatable !== false && (
          <Dialog open={isAiImportOpen} onOpenChange={setIsAiImportOpen}>
            <DialogTrigger asChild><Button variant="outline" size="sm" className="gap-2 border-purple-200 hover:bg-purple-50 text-purple-600"><Sparkles size={14} /> <span className="hidden sm:inline">{t('entity:import_ai')}</span></Button></DialogTrigger>
